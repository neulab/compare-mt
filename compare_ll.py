...
def print_word_likelihood_report(corpus, ll1, ll2, bucket_type='freq',
    ...
    Args:
    corpus: the corpus of words over which the likelihoods are computed
    ll1: likelihoods corresponding to each word in corpus from the sys 1
    ll2: likelihoods corresponding to each word in corpus from the sys 2
    ...
    ll1_out = bucketer.calc_bucketed_likelihoods(corpus, ll1)
    ll2_out = bucketer.calc_bucketed_likelihoods(corpus, ll2)

    print(f'--- avergae word log likelihood by {bucketer.name()} bucket')
    for bucket_str, l1, l2 in zip(bucketer.bucket_strs, ll1_out, ll2_out):
      print("{}\t{:.4f}\t{:.4f}".format(bucket_str, l1, l2))
    print()
...
if __name__ == '__main__':
    ...
    parser.add_argument('--corpus', type=str, dest='corpus',
    ...
    corpus = corpus_utils.load_tokens(args.corpus)
    ...
    if args.compare_word_likelihoods:
        print_header('Word Likelihood Analysis')
        for profile in args.compare_word_likelihoods:
            kargs = parse_profile(profile)
            print_word_likelihood_report(corpus, ll1, ll2, **kargs)
            print()